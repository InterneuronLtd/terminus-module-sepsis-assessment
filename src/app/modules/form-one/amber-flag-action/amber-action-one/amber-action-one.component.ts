...
@Component({
  selector: 'app-amber-action-one',
  templateUrl: './amber-action-one.component.html',
  styleUrls: ['./amber-action-one.component.css']
})
export class AmberActionOneComponent extends FormOneTaskBaseComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.assessmentTaskOneObj = super.retrieveAssessmentTask(this.actionMode,
      FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_8_id, this.assessment.assessmenttasks);

    this.taskOnedata = super.retrieveTaskData(this.actionMode, FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_8_id, this.assessmentTaskOneObj);
