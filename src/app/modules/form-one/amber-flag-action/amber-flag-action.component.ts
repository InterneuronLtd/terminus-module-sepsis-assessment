...
@Component({
  selector: 'app-amber-flag-action',
...
export class AmberFlagActionComponent extends FormActionBaseComponent implements OnInit, OnDestroy {
  ...
  getFormTaskMeta(onSucessfullFormTAskFetch: () => void) {
    ...
    const url = `${SepsisAssessmentModuleConfigData.Config.app_service.base_uri}/GetListByAttribute?synapsenamespace=meta&synapseentityname=sepsistask&synapseattributename=formsection_id&attributevalue=${FORM_ONE_CONFIG.enabled_section_id.section_4_id}`
    ...
    this.apiService.get(url)
      ...
      .subscribe((formTasksFromDB: any) => {
        ...
        if (!formTasks || !Array.isArray(formTasks) || formTasks.length == 0) {
          ...
          const templateVersionForTask1: number[] = formTasks.filter(ft => ft.sepsistask_id === FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_8_id)
  ...
  createNewAssessmentTasks() {
    ...
    let assessmentTaskSec1 = this.generateNewAssessmentTask(FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_8_id, this.amberTemplateVersionId);
  ...
  assignTaskTemplateVersionId(at: AssessmentTask) {
    if (at.sepsistask_id == FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_8_id) {
