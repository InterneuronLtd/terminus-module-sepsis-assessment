...
export class RedFlagActionComponent extends FormActionBaseComponent implements OnInit, OnDestroy {
  ...
  constructor(protected injector: Injector, private apiService: InAppBaseApiService, private formOneContextService: FormOneContextService, private formAPIService: FormAPIService) {
    super(injector);
  ...
  getFormTaskMeta(onSucessfullFormTAskFetch: () => void) {
    ...
    const url = `${SepsisAssessmentModuleConfigData.Config.app_service.base_uri}/GetListByAttribute?synapsenamespace=meta&synapseentityname=sepsistask&synapseattributename=formsection_id&attributevalue=${FORM_ONE_CONFIG.enabled_section_id.section_3_id}`
    ...
    this.apiService.get(url)
      ...
      .subscribe((formTasksFromDB: any) => {
        ...
        if (!formTasks || !Array.isArray(formTasks) || formTasks.length == 0) {
          ...
          const templateVersionForTask1: number[] = formTasks.filter(ft => ft.sepsistask_id === FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_1_id)
            .map(fs => fs.templateversionid);
          this.formtask1TemplateVersionId = Math.max(...templateVersionForTask1);

          const templateVersionForTask2: number[] = formTasks.filter(ft => ft.sepsistask_id === FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_2_id)
            .map(fs => fs.templateversionid);
          this.formtask2TemplateVersionId = Math.max(...templateVersionForTask2);

          const templateVersionForTask3: number[] = formTasks.filter(ft => ft.sepsistask_id === FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_3_id)
            .map(fs => fs.templateversionid);
          this.formtask3TemplateVersionId = Math.max(...templateVersionForTask3);

          const templateVersionForTask4: number[] = formTasks.filter(ft => ft.sepsistask_id === FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_4_id)
            .map(fs => fs.templateversionid);
          this.formtask4TemplateVersionId = Math.max(...templateVersionForTask4);

          const templateVersionForTask5: number[] = formTasks.filter(ft => ft.sepsistask_id === FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_5_id)
            .map(fs => fs.templateversionid);
          this.formtask5TemplateVersionId = Math.max(...templateVersionForTask5);

          const templateVersionForTask6: number[] = formTasks.filter(ft => ft.sepsistask_id === FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_6_id)
            .map(fs => fs.templateversionid);
          this.formtask6TemplateVersionId = Math.max(...templateVersionForTask6);

          const templateVersionForTask7: number[] = formTasks.filter(ft => ft.sepsistask_id === FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_7_id)
            .map(fs => fs.templateversionid);
  ...
  createNewAssessmentTasks() {
    ...
    let assessmentTaskSec1 = this.generateNewAssessmentTask(FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_1_id, this.formtask1TemplateVersionId);
    assessmentTaskSec1.assessmentversionid = this.assessment.versionid;
    this.assessment.assessmenttasks.push(assessmentTaskSec1);

    let assessmentTaskSec2 = this.generateNewAssessmentTask(FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_2_id, this.formtask2TemplateVersionId);
    assessmentTaskSec2.assessmentversionid = this.assessment.versionid;
    this.assessment.assessmenttasks.push(assessmentTaskSec2);

    let assessmentTaskSec3 = this.generateNewAssessmentTask(FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_3_id, this.formtask3TemplateVersionId);
    assessmentTaskSec3.assessmentversionid = this.assessment.versionid;
    this.assessment.assessmenttasks.push(assessmentTaskSec3);

    let assessmentTaskSec4 = this.generateNewAssessmentTask(FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_4_id, this.formtask4TemplateVersionId);
    assessmentTaskSec4.assessmentversionid = this.assessment.versionid;
    this.assessment.assessmenttasks.push(assessmentTaskSec4);

    let assessmentTaskSec5 = this.generateNewAssessmentTask(FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_5_id, this.formtask5TemplateVersionId);
    assessmentTaskSec5.assessmentversionid = this.assessment.versionid;
    this.assessment.assessmenttasks.push(assessmentTaskSec5);

    let assessmentTaskSec6 = this.generateNewAssessmentTask(FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_6_id, this.formtask6TemplateVersionId);
    assessmentTaskSec6.assessmentversionid = this.assessment.versionid;
    this.assessment.assessmenttasks.push(assessmentTaskSec6);

    let assessmentTaskSec7 = this.generateNewAssessmentTask(FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_7_id, this.formtask7TemplateVersionId);
  ...
  assignTaskTemplateVersionId(at: AssessmentTask) {
    if (at.sepsistask_id == FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_1_id) {
      this.formtask1TemplateVersionId = at.sepsistasktemplateversionid;
    } else if (at.sepsistask_id == FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_2_id) {
      this.formtask2TemplateVersionId = at.sepsistasktemplateversionid;
    } else if (at.sepsistask_id == FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_3_id) {
      this.formtask3TemplateVersionId = at.sepsistasktemplateversionid;
    } else if (at.sepsistask_id == FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_4_id) {
      this.formtask4TemplateVersionId = at.sepsistasktemplateversionid;
    } else if (at.sepsistask_id == FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_5_id) {
      this.formtask5TemplateVersionId = at.sepsistasktemplateversionid;
    } else if (at.sepsistask_id == FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_6_id) {
      this.formtask6TemplateVersionId = at.sepsistasktemplateversionid;
    } else if (at.sepsistask_id == FORM_ONE_CONFIG.enabled_sepsis_task_id.sepsistask_7_id) {
      this.formtask7TemplateVersionId = at.sepsistasktemplateversionid;
    }
  }

  confirmSave() {
    this.formOneContextService.errorCount = 0;
    this.formOneContextService.formValidation.next("Isvalide");
    if (this.formOneContextService.errorCount == 0) {
  ...
  preparePostSaveActionBasedOnStatus(status: string, showNextAction?: boolean) {
    ...
    this.postSaveAction(this.saveStatusMessagePopup, FORM_ONE_CONFIG.sepsistask_common_config.messages.SuccessSaveMessage,
      FORM_ONE_CONFIG.sepsistask_common_config.messages.ErrorSaveMessage,
      () => {
