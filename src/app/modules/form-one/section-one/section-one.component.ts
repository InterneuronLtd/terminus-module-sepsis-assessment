...
import { OptionCheckboxListComponentModel } from '../../shared/components/option-checkbox/option-checkbox-list.component';
import { getAllOptionTypeProperties, optionTypeMetadataKey, getOptionTextForProperty } from '../../shared/services/decorators/option-type.decorator';
...
export class SectionOneComponent extends FormOneSectionBaseComponent implements OnInit, OnDestroy {
  ...
  constructor(protected injector: Injector, 
    ...
    this.saveConfirmation(this.confirmationMessagePopup, FORM_ONE_CONFIG.section_1_config.messages.SaveConfirmationMessage,
      () => {
  ...
  ngOnInit() {
    ...
    this.sectionOneAssessmentDetail = super.retrieveSectionAssessmentDetail(this.actionMode,
      FORM_ONE_CONFIG.enabled_section_id.section_1_id, this.assessment.assessmentdetails);

    this.originalSectionDataObj = this.sectionDataObj = super.retrieveSectionData(this.actionMode, FORM_ONE_CONFIG.section_1_config.question_1_id, this.sectionOneAssessmentDetail);
    ...
    this.formOneContextService.displaySectionTwo = this.sectionOneVM.selectedYesNo == 'YES';

    this.showSubmitPanel = (!this.readOnly && this.sectionOneVM.selectedYesNo == 'NO');

    this.loggerService.log('inside section one');
    this.loggerService.log(this.formOneContextService);
  ...
  buildOptionList(): any {
    ...
    let optionCheckboxListComponentModels: Array<OptionCheckboxListComponentModel> = new Array<OptionCheckboxListComponentModel>();//

    //this.loggerService.log(getFilteredProperties(this.sectionOneVM));
    //console.log(myDecoratorUsingClass(SectionOneVM, 'Impairedimmunity')); // true


    // this.loggerService.log(!!Reflect.getMetadata(optionTypeMetadataKey, SectionOneVM, 'Impairedimmunity'));

    let vmKeys = getAllOptionTypeProperties(this.sectionOneVM);//Object.keys(this.sectionOneVM);
    //let vmKeys = Object.keys(this.sectionOneVM);

    vmKeys.forEach(vmKey => {

      this.loggerService.log(vmKey);

      this.loggerService.log(Reflect.hasMetadata(optionTypeMetadataKey, this.sectionOneVM, vmKey));
      //this.loggerService.log(Reflect.getMetadata(optionTypeMetadataKey, this.sectionOneVM));

      //if (Reflect.hasMetadata(optionTypeMetadataKey, this.sectionOneVM, vmKey)) {
      let optionMetaData = getOptionTextForProperty(this.sectionOneVM, vmKey);;
      let model: OptionCheckboxListComponentModel = new OptionCheckboxListComponentModel();
      model.key = vmKey;
      model.optionModel = this.sectionOneVM;
      model.text = optionMetaData.text;

      this.loggerService.log('model Data=');
      this.loggerService.log(model);
      optionCheckboxListComponentModels.push(model);
      //}
    });

    this.loggerService.log('OptionCheckboxListComponentModel');
    this.loggerService.log(optionCheckboxListComponentModels);
  }
  ...
  onDecisionChange(event: any) {
    ...
    if (this.sectionOneVM.selectedYesNo !== 'YES') {
      this.formOneContextService.displaySectionThree = false;
  ...
  resetOtherDependentSectionsData(): any {
    ...
    let dependentAssessmentDetails: AssessmentDetail[] = this.assessment.assessmentdetails.filter(ad =>
      ad.formsection_id == FORM_ONE_CONFIG.enabled_section_id.section_4_id || ad.formsection_id == FORM_ONE_CONFIG.enabled_section_id.section_3_id || ad.formsection_id == FORM_ONE_CONFIG.enabled_section_id.section_2_id);
  ...
  preparePostSaveActionBasedOnStatus(status: string, showNextAction?: boolean) {
    ...
    this.postSaveAction(this.saveStatusMessagePopup, FORM_ONE_CONFIG.section_1_config.messages.SuccessSaveMessage,
      FORM_ONE_CONFIG.section_1_config.messages.ErrorSaveMessage,
      () => {
