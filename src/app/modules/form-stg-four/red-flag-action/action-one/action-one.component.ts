...
@Component({
  selector: 'app-form4-action-one',
  templateUrl: './action-one.component.html',
  styleUrls: ['./action-one.component.css']
})
export class ActionOneComponent extends FormFourTaskBaseComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.assessmentTaskOneObj = super.retrieveAssessmentTask(this.actionMode,
      FORM_STG_FOUR_CONFIG.enabled_sepsis_task_id.sepsistask_1_id, this.assessment.assessmenttasks);

    this.taskOnedata = super.retrieveTaskData(this.actionMode, FORM_STG_FOUR_CONFIG.enabled_sepsis_task_id.sepsistask_1_id, this.assessmentTaskOneObj);
