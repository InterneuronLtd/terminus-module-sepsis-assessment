...
@Component({
  selector: 'app-form4-action-six',
  templateUrl: './action-six.component.html',
  styleUrls: ['./action-six.component.css']
})
export class ActionSixComponent extends FormFourTaskBaseComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.taskSixDataObj = super.retrieveAssessmentTask(this.actionMode,
      FORM_STG_FOUR_CONFIG.enabled_sepsis_task_id.sepsistask_6_id, this.assessment.assessmenttasks);

    this.originalTaskDataObj = this.taskSixdata = super.retrieveTaskData(this.actionMode, FORM_STG_FOUR_CONFIG.enabled_sepsis_task_id.sepsistask_6_id, this.taskSixDataObj);
