...
@Component({
  selector: 'app-form4-action-three',
  templateUrl: './action-three.component.html',
  styleUrls: ['./action-three.component.css']
})
export class ActionThreeComponent extends FormFourTaskBaseComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.taskThreeDataObj = super.retrieveAssessmentTask(this.actionMode,
      FORM_STG_FOUR_CONFIG.enabled_sepsis_task_id.sepsistask_3_id, this.assessment.assessmenttasks);

    this.originalTaskDataObj = this.taskThreedata = super.retrieveTaskData(this.actionMode, FORM_STG_FOUR_CONFIG.enabled_sepsis_task_id.sepsistask_3_id, this.taskThreeDataObj);
