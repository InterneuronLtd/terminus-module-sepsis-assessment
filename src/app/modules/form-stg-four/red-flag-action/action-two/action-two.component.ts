...
@Component({
  selector: 'app-form4-action-two',
  templateUrl: './action-two.component.html',
  styleUrls: ['./action-two.component.css']
})
export class ActionTwoComponent extends FormFourTaskBaseComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.tasktwoDataObj = super.retrieveAssessmentTask(this.actionMode,
      FORM_STG_FOUR_CONFIG.enabled_sepsis_task_id.sepsistask_2_id, this.assessment.assessmenttasks);

    this.originalTaskDataObj = this.tasktwodata = super.retrieveTaskData(this.actionMode, FORM_STG_FOUR_CONFIG.enabled_sepsis_task_id.sepsistask_2_id, this.tasktwoDataObj);
