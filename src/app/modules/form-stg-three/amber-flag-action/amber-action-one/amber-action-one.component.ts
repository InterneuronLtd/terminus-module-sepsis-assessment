...
@Component({
  selector: 'app-form3-amber-action-one',
  templateUrl: './amber-action-one.component.html',
  styleUrls: ['./amber-action-one.component.css']
})
export class AmberActionOneComponent extends FormThreeTaskBaseComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.assessmentTaskOneObj = super.retrieveAssessmentTask(this.actionMode,
      FORM_STG_THREE_CONFIG.enabled_sepsis_task_id.sepsistask_8_id, this.assessment.assessmenttasks);

    this.taskOnedata = super.retrieveTaskData(this.actionMode, FORM_STG_THREE_CONFIG.enabled_sepsis_task_id.sepsistask_8_id, this.assessmentTaskOneObj);
