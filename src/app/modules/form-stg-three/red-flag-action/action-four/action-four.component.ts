...
@Component({
  selector: 'app-form3-action-four',
  templateUrl: './action-four.component.html',
  styleUrls: ['./action-four.component.css']
})
export class ActionFourComponent extends FormThreeTaskBaseComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    this.taskFourDataObj = super.retrieveAssessmentTask(this.actionMode,
      FORM_STG_THREE_CONFIG.enabled_sepsis_task_id.sepsistask_4_id, this.assessment.assessmenttasks);

    this.originalTaskDataObj = this.taskFourdata = super.retrieveTaskData(this.actionMode, FORM_STG_THREE_CONFIG.enabled_sepsis_task_id.sepsistask_4_id, this.taskFourDataObj);
