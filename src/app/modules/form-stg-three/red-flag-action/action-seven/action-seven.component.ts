...
@Component({
  selector: 'app-form3-action-seven',
  templateUrl: './action-seven.component.html',
  styleUrls: ['./action-seven.component.css']
})
export class ActionSevenComponent extends FormThreeTaskBaseComponent implements OnInit {
  ...
  constructor(public formStageThreeContextService: FormStageThreeContextService, protected injector: Injector, private formAPIService: FormAPIService) {
    super(injector);
  ...
  ngOnInit() {
    this.taskSevenDataObj = super.retrieveAssessmentTask(this.actionMode,
      FORM_STG_THREE_CONFIG.enabled_sepsis_task_id.sepsistask_7_id, this.assessment.assessmenttasks);

    this.originalTaskDataObj = this.taskSevendata = super.retrieveTaskData(this.actionMode, FORM_STG_THREE_CONFIG.enabled_sepsis_task_id.sepsistask_7_id, this.taskSevenDataObj);
