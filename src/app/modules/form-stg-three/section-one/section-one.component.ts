...
@Component({
  selector: 'app-form3-section-one',
  templateUrl: './section-one.component.html',
  styleUrls: ['./section-one.component.css']
})
export class SectionOneComponent extends FormStageThreeSectionBaseComponent implements OnInit, OnDestroy {
  ...
  constructor(protected injector: Injector, 
    private formThreeContextService: FormStageThreeContextService, 
    private formAPIService: FormAPIService) {

    super(injector);

    this.saveConfirmation(this.confirmationMessagePopup, FORM_STG_THREE_CONFIG.section_1_config.messages.SaveConfirmationMessage,
      () => {
  ...
  ngOnInit() {
    ...
    this.sectionOneAssessmentDetail = super.retrieveSectionAssessmentDetail(this.actionMode,
      FORM_STG_THREE_CONFIG.enabled_section_id.section_1_id, this.assessment.assessmentdetails);

    this.originalSectionDataObj = this.sectionDataObj = super.retrieveSectionData(this.actionMode, FORM_STG_THREE_CONFIG.section_1_config.question_1_id, this.sectionOneAssessmentDetail);
    ...
    this.formThreeContextService.displaySectionTwo = this.sectionOneVM.selectedYesNo == 'YES';
  ...
  onDecisionChange(event: any) {
    ...
    if (this.sectionOneVM.selectedYesNo !== 'YES') {
      this.formThreeContextService.displaySectionThree = false;
      this.formThreeContextService.displaySectionFour = false;
  ...
  resetOtherDependentSectionsData(): any {
    ...
    let dependentAssessmentDetails: AssessmentDetail[] = this.assessment.assessmentdetails.filter(ad =>
      ad.formsection_id == FORM_STG_THREE_CONFIG.enabled_section_id.section_4_id || ad.formsection_id == FORM_STG_THREE_CONFIG.enabled_section_id.section_3_id || ad.formsection_id == FORM_STG_THREE_CONFIG.enabled_section_id.section_2_id);
  ...
  preparePostSaveActionBasedOnStatus(status: string, showNextAction?: boolean) {
    ...
    this.postSaveAction(this.saveStatusMessagePopup, FORM_STG_THREE_CONFIG.section_1_config.messages.SuccessSaveMessage,
      FORM_STG_THREE_CONFIG.section_1_config.messages.ErrorSaveMessage,
      () => {
