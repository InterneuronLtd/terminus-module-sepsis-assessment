...
import { SectionTwoVM } from './section-two.model';
import { AssessmentDetail, AssessmentEntity, SectionData, Answer } from 'src/app/models/Assessment.model';
...
@Component({
  selector: 'app-form3-section-two',
  templateUrl: './section-two.component.html',
  styleUrls: ['./section-two.component.css']
})
export class SectionTwoComponent extends FormStageThreeSectionBaseComponent implements OnInit, OnDestroy {
  readOnly = false;
  _actionMode: string;
  ...
  constructor(protected injector: Injector,
    private formStgThreeContextService: FormStageThreeContextService,
    private formAPIService: FormAPIService
  ) {
    super(injector);

    this.saveConfirmation(this.confirmationMessagePopup, FORM_STG_THREE_CONFIG.section_2_config.messages.SaveConfirmationMessage,
      () => {
  ...
  ngOnInit() {
    ...
    this.sectionTwoAssessmentDetail = super.retrieveSectionAssessmentDetail(this.actionMode,
      FORM_STG_THREE_CONFIG.enabled_section_id.section_2_id, this.assessment.assessmentdetails);

    this.originalSectionDataObj = this.sectionDataObj = super.retrieveSectionData(this.actionMode, FORM_STG_THREE_CONFIG.section_2_config.question_1_id, this.sectionTwoAssessmentDetail);
    ...
    this.formStgThreeContextService.displaySectionThree = this.sectionTwoVM.selectedYesNo === 'YES';
  ...
  submitForm() {
    ...
    this.formAPIService.saveAssessment(this.actionMode, this.assessment,
      (newAssessment) => {
        ...
        let section4AssessmentDetail: AssessmentDetail = this.assessment.assessmentdetails.find(ad => ad.formsection_id == FORM_STG_THREE_CONFIG.enabled_section_id.section_4_id);
  ...
  onDecisionChange(event: any) {
    ...
    if (this.sectionTwoVM.selectedYesNo != 'YES') {
      this.formStgThreeContextService.displaySectionFour = false;
  ...
  resetOtherDependentSectionsData(): any {
    ...
    let dependentAssessmentDetails: AssessmentDetail[] = this.assessment.assessmentdetails.filter(ad =>
      ad.formsection_id == FORM_STG_THREE_CONFIG.enabled_section_id.section_4_id || ad.formsection_id == FORM_STG_THREE_CONFIG.enabled_section_id.section_3_id);
  ...
  preparePostSaveActionBasedOnStatus(status: string, showNextAction?: boolean) {
    ...
    this.postSaveAction(this.saveStatusMessagePopup, FORM_STG_THREE_CONFIG.section_2_config.messages.SuccessSaveMessage,
      FORM_STG_THREE_CONFIG.section_2_config.messages.ErrorSaveMessage,
      () => {
