...
@Component({
  selector: 'app-form-stg-two',
  templateUrl: './form-stg-two.component.html',
  styleUrls: ['./form-stg-two.component.css']
})
export class FormStageTwoComponent extends FormBaseComponent implements OnInit, OnDestroy {

  mappedFormTypeId = FORM_STG_TWO_CONFIG.form_type_id;
  ...
  ngOnInit() {
    this.loggerService.log('Showing Form for Age 5-12');
  }

  ngOnDestroy() {
    this.loggerService.log('Destroying Form for Age 5-12');

    this.assessment = null;
    this.assessmentContext = null;

    if (this.formStageTwoContextService) this.formStageTwoContextService.onDestroy();

    this.formStageTwoContextService = null;
  ...
  assignTemplateVersionId(ad: AssessmentDetail) {
    if (ad.formsection_id == FORM_STG_TWO_CONFIG.enabled_section_id.section_1_id) {
      this.formSection1TemplateVersionId = ad.sectiontemplateversionid;
    } else if (ad.formsection_id == FORM_STG_TWO_CONFIG.enabled_section_id.section_2_id) {
      this.formSection2TemplateVersionId = ad.sectiontemplateversionid;
    } else if (ad.formsection_id == FORM_STG_TWO_CONFIG.enabled_section_id.section_3_id) {
      this.formSection3TemplateVersionId = ad.sectiontemplateversionid;
    } else if (ad.formsection_id == FORM_STG_TWO_CONFIG.enabled_section_id.section_4_id) {
  ...
  handleNewAssessment(onLoadComplete?: () => void) {
    ...
    let confiruredSections = [FORM_STG_TWO_CONFIG.enabled_section_id.section_1_id, FORM_STG_TWO_CONFIG.enabled_section_id.section_2_id, FORM_STG_TWO_CONFIG.enabled_section_id.section_3_id, FORM_STG_TWO_CONFIG.enabled_section_id.section_4_id];
  ...
  loadAssessmentTask(onLoadComplete?: () => void) {
    this.taskModeAssessmentModelBuilderService.buildAssessmentModel(this.assessmentContext, this.mappedFormTypeId, null,
      (assessmentFromDB) => {
        ...
        if (this.assessment.taskformsectionid == FORM_STG_TWO_CONFIG.enabled_section_id.section_3_id) {
          this.showFormSections = false;
          this.showTasKSections = true;
          this.showRedFlagTasKSections = true;
        }
        else if (this.assessment.taskformsectionid == FORM_STG_TWO_CONFIG.enabled_section_id.section_4_id) {
          this.showFormSections = false;
          this.showTasKSections = true;
