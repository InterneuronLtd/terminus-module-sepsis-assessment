...
@Component({
  selector: 'app-form2-action-four',
  templateUrl: './action-four.component.html',
  styleUrls: ['./action-four.component.css']
})
export class ActionFourComponent extends FormTwoTaskBaseComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    this.taskFourDataObj = super.retrieveAssessmentTask(this.actionMode,
      FORM_STG_TWO_CONFIG.enabled_sepsis_task_id.sepsistask_4_id, this.assessment.assessmenttasks);

    this.originalTaskDataObj = this.taskFourdata = super.retrieveTaskData(this.actionMode, FORM_STG_TWO_CONFIG.enabled_sepsis_task_id.sepsistask_4_id, this.taskFourDataObj);
