...
@Component({
  selector: 'app-form2-action-three',
  templateUrl: './action-three.component.html',
  styleUrls: ['./action-three.component.css']
})
export class ActionThreeComponent extends FormTwoTaskBaseComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    this.taskThreeDataObj = super.retrieveAssessmentTask(this.actionMode,
      FORM_STG_TWO_CONFIG.enabled_sepsis_task_id.sepsistask_3_id, this.assessment.assessmenttasks);

    this.originalTaskDataObj = this.taskThreedata = super.retrieveTaskData(this.actionMode, FORM_STG_TWO_CONFIG.enabled_sepsis_task_id.sepsistask_3_id, this.taskThreeDataObj);
