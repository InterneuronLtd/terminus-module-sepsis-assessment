...
@Component({
  selector: 'app-form2-action-two',
  templateUrl: './action-two.component.html',
  styleUrls: ['./action-two.component.css']
})
export class ActionTwoComponent extends FormTwoTaskBaseComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    this.tasktwoDataObj = super.retrieveAssessmentTask(this.actionMode,
      FORM_STG_TWO_CONFIG.enabled_sepsis_task_id.sepsistask_2_id, this.assessment.assessmenttasks);

    this.originalTaskDataObj = this.tasktwodata = super.retrieveTaskData(this.actionMode, FORM_STG_TWO_CONFIG.enabled_sepsis_task_id.sepsistask_2_id, this.tasktwoDataObj);
