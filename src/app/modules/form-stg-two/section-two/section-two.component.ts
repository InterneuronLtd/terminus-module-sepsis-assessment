...
@Component({
  selector: 'app-form2-section-two',
  templateUrl: './section-two.component.html',
  styleUrls: ['./section-two.component.css']
})
export class SectionTwoComponent extends FormStageTwoSectionBaseComponent implements OnInit, OnDestroy {
  readOnly = false;
  _actionMode: string;
  ...
  constructor(protected injector: Injector,
    private formStgTwoContextService: FormStageTwoContextService,
    private formAPIService: FormAPIService
  ) {
    super(injector);

    this.saveConfirmation(this.confirmationMessagePopup, FORM_STG_TWO_CONFIG.section_2_config.messages.SaveConfirmationMessage,
      () => {
  ...
  ngOnInit() {
    ...
    this.sectionTwoAssessmentDetail = super.retrieveSectionAssessmentDetail(this.actionMode,
      FORM_STG_TWO_CONFIG.enabled_section_id.section_2_id, this.assessment.assessmentdetails);

    this.originalSectionDataObj = this.sectionDataObj = super.retrieveSectionData(this.actionMode, FORM_STG_TWO_CONFIG.section_2_config.question_1_id, this.sectionTwoAssessmentDetail);
    ...
    this.formStgTwoContextService.displaySectionThree = this.sectionTwoVM.selectedYesNo === 'YES';
  ...
  onDecisionChange(event: any) {
    ...
    if (this.sectionTwoVM.selectedYesNo != 'YES') {
      this.formStgTwoContextService.displaySectionFour = false;
  ...
  resetOtherDependentSectionsData(): any {
    ...
    let dependentAssessmentDetails: AssessmentDetail[] = this.assessment.assessmentdetails.filter(ad =>
      ad.formsection_id == FORM_STG_TWO_CONFIG.enabled_section_id.section_4_id || ad.formsection_id == FORM_STG_TWO_CONFIG.enabled_section_id.section_3_id);
